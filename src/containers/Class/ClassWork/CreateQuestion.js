...
const CreateQuestion = ({ onCancel, onDone }) => {
  ...
  return (
    <Box className="createQuestion">
      <Typography sx={{ fontWeight: 'bold' }}>Add New Question</Typography>
      <TextField
        margin="normal"
      ...
      <FormControl fullWidth size="small" margin="normal">
        <InputLabel id="deliverable-type-label">Question Type</InputLabel>
        <Select
          label="Question Type"
          id="question-type"
          value={type}
        ...
        >
          ...
          <MenuItem value={questionTypes.trueFalse}>True/False</MenuItem>
          <MenuItem value={questionTypes.single}>Single Answer</MenuItem>
          <MenuItem value={questionTypes.multiple}>Multiple Answers</MenuItem>
        </Select>
      </FormControl>
      {showAnswers && (
        <Box>
          ...
          />
          {choices &&
            [...Array(numberChoices ? numberChoices : 2).keys()].map(
              (choice) => (
                <TextField
                  key={choice}
                  margin="normal"
